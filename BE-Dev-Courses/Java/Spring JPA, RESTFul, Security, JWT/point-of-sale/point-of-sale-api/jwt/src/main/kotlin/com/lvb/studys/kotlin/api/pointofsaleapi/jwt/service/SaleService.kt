...
import com.lvb.studys.kotlin.api.pointofsaleapi.jwt.dto.ProductInfoDTO
import com.lvb.studys.kotlin.api.pointofsaleapi.jwt.dto.ProductSaleDTO
import com.lvb.studys.kotlin.api.pointofsaleapi.jwt.dto.SaleDTO
import com.lvb.studys.kotlin.api.pointofsaleapi.jwt.dto.SaleInfoDTO
...
import com.lvb.studys.kotlin.api.pointofsaleapi.jwt.repository.ItemSaleRepository
import com.lvb.studys.kotlin.api.pointofsaleapi.jwt.repository.ProductRepository
import com.lvb.studys.kotlin.api.pointofsaleapi.jwt.repository.SaleRepository
...
class SaleService @Autowired constructor(
    ...
    fun saveSale(saleDto: SaleDTO) : Long {
        ...
        val user = saleDto.userId?.let { userRepository.findById(it).orElseThrow { throw NoItemException("User not Found") } }
    ...
    private fun getItemSale(products: List<ProductSaleDTO>): List<ItemSale> {
        ...
        return products.map {
            val product = productRepository.findById(it.productId).orElseThrow {throw NoItemException("Item not Found") }

            if (product.quantity == 0) {
                throw NoItemException("Product without stock")
            } else if ((product.quantity ?: it.quantity) < it.quantity) {
                throw InvalidOperationException(String.format("The quantity of sale items (%s) is greater than the quantity available in stock (%s)", it.quantity, product.quantity))
            }
